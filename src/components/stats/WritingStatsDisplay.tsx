...
export const WritingStatsDisplay: React.FC<WritingStatsDisplayProps> = ({
  ...
  return (
    <div className={`writing-stats flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400 px-4 py-2 ${showBorder ? 'border-t border-gray-200/50 dark:border-white/5' : ''} ${className}`}>
      <div className="stat-item flex items-center gap-1.5" title={t('stats_characters')}>
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M4 7V4h16v3M9 20h6M12 4v16" />
        </svg>
        <span>{stats.characters.toLocaleString()}</span>
      </div>
      <div className="stat-item flex items-center gap-1.5" title={t('stats_words')}>
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M4 19h16M4 15l4-11 4 11 4-11 4 11" />
        </svg>
        <span>{stats.words.toLocaleString()}</span>
      </div>
      <div className="stat-item flex items-center gap-1.5 hidden md:flex" title={t('stats_paragraphs')}>
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className="stat-item flex items-center gap-1.5 hidden lg:flex" title={t('stats_readingTime')}>
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <circle cx="12" cy="12" r="10" />
          <polyline points="12 6 12 12 16 14" />
        </svg>
        <span>{t('stats_minutes', { count: stats.readingTimeMinutes })}</span>
      </div>
    </div>
  );
};
