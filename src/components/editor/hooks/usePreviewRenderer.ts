  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (embeds.length === 0) {
          if (!cancelled) {
            const nextHtml = restoreShikiPresFromSnapshots(
              host.innerHTML,
              shikiSnapshots,
            );
        ...
        await Promise.all(
          embeds.map(async (embed) => {
            try {
              const target =
                embed.dataset.wikiTarget?.trim() ||
                embed.dataset.wikilink?.trim();
              const label =
                embed.dataset.wikiLabel?.trim() ||
              ...
              if (isPdfAttachment(resolvedTarget.name)) {
                try {
                  ...
                  if (embedHeight)
                    pdfContainer.style.height = `${embedHeight}px`;
