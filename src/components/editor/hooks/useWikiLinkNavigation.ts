...
// Helper functions
function findHeadingDefinitionByReference(
  ...
  const headingCandidates = Array.from(
    new Set([normalizedReference, createHeadingSlug(normalizedReference)]),
  );
...
function findHeadingElementByReference(
  container: HTMLElement | null,
  rawReference: string,
): HTMLElement | null {
  ...
  const byDataAttr = Array.from(
    container.querySelectorAll<HTMLElement>(
      "article.markdown-body [data-heading-id]",
    ),
  ).find((element) => {
    ...
    return (
      headingCandidates.includes(headingId) ||
  ...
  const byId = container.querySelector<HTMLElement>(
    `article.markdown-body [id="${CSS.escape(normalizedReference)}"]`,
  );
  if (byId) return byId;

  // Third try: find by slugified id
  const bySlugId = container.querySelector<HTMLElement>(
    `article.markdown-body [id="${CSS.escape(createHeadingSlug(normalizedReference))}"]`,
  );
  ...
  return (
    Array.from(
      ...
      return (
        headingCandidates.includes(text) || headingCandidates.includes(textSlug)
      );
    }) ?? null
  );
}

function findBlockElementByReference(
  container: HTMLElement | null,
  rawReference: string,
): HTMLElement | null {
  if (!container) return null;
  ...
  return container.querySelector<HTMLElement>(
    `article.markdown-body [data-block-id="${CSS.escape(normalizedReference)}"]`,
  );
}

function scrollContainerToHeading(
  ...
  if (options?.alignMode === "center") {
    ...
    const alignTopRatio = Math.min(
      Math.max(options?.alignTopRatio ?? 0.12, 0, 1),
      1,
    );
...
export function useWikiLinkNavigation(
  ...
  const flattenedHeadings = useMemo(
    () => flattenHeadingNodes(parseHeadings(content)),
    [content],
  );
  ...
  const scrollToReference = useCallback(
    (referenceId: string, scrollOptions?: HeadingScrollOptions): boolean => {
      ...
      const attemptScroll = (rawReference?: string): boolean => {
        ...
        let target =
          findHeadingElementByReference(container, referenceId) ||
      ...
      headingScrollAnimationFrameRef.current = requestAnimationFrame(() => {
        ...
        headingScrollTimeoutRefs.current = HEADING_SCROLL_RETRY_DELAYS_MS.map(
          (delay) =>
            window.setTimeout(() => {
  ...
  const navigateToWikilink = useCallback(
    async (wikiTarget: string): Promise<boolean> => {
      ...
      if (!parsedReference.subpathType && parsedReference.path.trim()) {
        const matchedHeading = findHeadingDefinitionByReference(
          flattenedHeadings,
          wikiTarget,
        );
        if (matchedHeading) {
      ...
      if (!parsedReference.path.trim() && explicitReferenceTarget) {
        ...
        const canResolve = Boolean(
          findHeadingDefinitionByReference(
            flattenedHeadings,
            explicitReferenceTarget,
          ) ||
        ...
        scrollToReference(
          explicitReferenceTarget,
          CENTERED_HEADING_SCROLL_OPTIONS,
        );
        return true;
      }
      ...
      if (wikiTarget.trim().startsWith("#")) {
        const container = containerRef.current;
        const matchedElement = findHeadingElementByReference(
          container,
          wikiTarget,
        );
        ...
        if (matchedBlock) {
          scrollToReference(
            matchedBlock.dataset.blockId ?? wikiTarget,
            CENTERED_HEADING_SCROLL_OPTIONS,
          );
          return true;
        }
        ...
        scrollToReference(
          matchedElement.dataset.headingId ?? matchedElement.id,
          CENTERED_HEADING_SCROLL_OPTIONS,
        );
        return true;
      }
  ...
  const navigateToHashLink = useCallback(
    (normalizedHash: string): boolean => {
      const container = containerRef.current;
      const blockElement = findBlockElementByReference(
        container,
        normalizedHash,
      );
      if (blockElement) {
        return (
          scrollToReference(
            blockElement.dataset.blockId ?? normalizedHash,
            CENTERED_HEADING_SCROLL_OPTIONS,
          ) || true
        );
      }
      ...
      if (normalizedHash.trim().startsWith("#")) {
        const fallbackHeading = findHeadingDefinitionByReference(
          flattenedHeadings,
          normalizedHash.trim().slice(1),
        );
        if (fallbackHeading) {
          return (
            scrollToReference(
      ...
      if (!matchedElement && normalizedHash.trim().startsWith("#")) {
        const fallbackElement = findHeadingElementByReference(
          container,
          normalizedHash.trim().slice(1),
        );
        ...
        return (
          scrollToReference(
            fallbackElement.dataset.headingId ?? fallbackElement.id,
            CENTERED_HEADING_SCROLL_OPTIONS,
          ) || true
        );
      }
