...
describe('nested list edit to preview integration', () => {
  ...
  it('Tab-nested unordered sibling renders nested ul in preview', () => {
    ...
    const next = applyCommand(handleListTab(), doc, childLineStart + 2, childLineStart + 2);

    expect(next.doc.toString()).toBe('- parent\n    - child');
    expect(previewHtml(next.doc.toString()).match(/<ul\b/g)?.length ?? 0).toBeGreaterThanOrEqual(2);
  });
