...
import { protectShikiPresInHtmlString, restoreShikiPresFromSnapshots } from './shikiHtmlSnapshots';
...
describe('shikiHtmlSnapshots', () => {
  it('round-trips Shiki pre HTML without relying on DOM serialization', () => {
    ...
    expect(restoreShikiPresFromSnapshots(fakeDomReadback, snapshots)).toBe(html);
  });
  ...
  it('handles multiple blocks and note-style concatenation', () => {
    ...
    html = protectShikiPresInHtmlString(`${html}<article>${b}</article>`, snapshots);
  ...
  it('tolerates missing slots during restore', () => {
    ...
    const snapshots: string[] = [];
    const protectedHtml = protectShikiPresInHtmlString(`<div>${pre}</div>`, snapshots);
    expect(snapshots).toHaveLength(1);

    // Simulate a DOM pipeline that drops the placeholder entirely.
    const dropped = protectedHtml.replace(/<div data-mp-shiki-slot="0"[^>]*><\/div>/, '');
