...
describe("parseHeadings", () => {
  ...
  it("ignores # lines inside fenced code blocks", () => {
    ...
    expect(flat.map((h) => h.text)).toEqual(["Real Title", "Real Section"]);
    expect(flat[1].line).toBe(8);
  });

  it("ignores headings inside tilde fences and resumes after close", () => {
    const content = "~~~\n# hidden\n~~~\n# Visible";
    const headings = parseHeadings(content);
    expect(headings).toHaveLength(1);
    expect(headings[0].text).toBe("Visible");
