...
const translations = {
  'zh-CN': {
    ...
    settings_formatOnManualSaveDesc: '手动保存时自动整理 Markdown：压缩多余空行，补齐标题、列表、引用、分隔线、表格、链接定义、脚注定义和 HTML 注释前后的标准空行，统一标题、列表、任务列表、引用和分隔线写法，并按当前列表模式整理有序列表编号，同时为正文中的中英文相邻内容补充空格。不会处理 frontmatter、围栏代码块和行内代码。',
