...
describe("buildExportStyles", () => {
  ...
  it("scopes export theme colors to .export-document[data-theme] for off-screen rasterization", () => {
    ...
    // var() references are resolved for html2canvas compatibility.
    expect(css).toMatch(
      /\.export-document\[data-theme="light"\] \.markdown-body\s*\{[^}]*--color-fg-default:\s*#[0-9a-fA-F]{3,8}/m,
    );
    expect(css).toMatch(
      /\.export-document\[data-theme="dark"\] \.markdown-body\s*\{[^}]*--color-fg-default:\s*#[0-9a-fA-F]{3,8}/m,
    );
