...
def main():
    ...
    add_text(slide, "M記 · 功能介绍",
             Inches(1.05), Inches(0.45), Inches(8), Inches(0.35),
             font=SANS, size=12, color=STONE)
    add_text(slide, "M記把本地 Markdown 写作、知识库导航和发布流程收在一个桌面编辑器里",
             Inches(1.05), Inches(0.95), Inches(11.2), Inches(1.05),
             font=SERIF, size=28, color=NEAR_BLACK)
    add_line(slide, Inches(1.05), Inches(2.25), Inches(11.15), color=BORDER, weight_pt=0.8)
    add_line(slide, Inches(6.65), Inches(2.55), Inches(0), color=BORDER, weight_pt=0.8)

    modules = [
        ("A", "写作视图", "编辑、分屏、预览三态切换；Outline、联动滚动和状态栏服务长文写作。"),
        ("B", "知识库链接", "支持 frontmatter、Obsidian wiki link、块引用、附件嵌入和图片粘贴落盘。"),
        ("C", "文件工作台", "本地文件夹作为知识库；多标签、全文搜索、拖拽移动、Trash 与附件清理覆盖日常维护。"),
        ("D", "AI 与发布", "双 Provider AI 增强内容；PDF、simple-blog 与微信公众号草稿打通交付出口。"),
    ]
    positions = [
        (Inches(1.05), Inches(2.65)),
        (Inches(6.9), Inches(2.65)),
        (Inches(1.05), Inches(4.45)),
        (Inches(6.9), Inches(4.45)),
    ]

    for (letter, title, body), (x, y) in zip(modules, positions):
        add_text(slide, letter, x, y, Inches(0.35), Inches(0.45),
                 font=SERIF, size=24, color=BRAND)
        add_text(slide, title, x + Inches(0.45), y + Inches(0.05), Inches(4.7), Inches(0.4),
                 font=SERIF, size=18, color=NEAR_BLACK)
        add_text(slide, body, x, y + Inches(0.62), Inches(4.85), Inches(0.88),
                 font=SANS, size=13, color=DARK_WARM)

    add_text(slide, "定位：比 Typora 更贴近知识库，比 Obsidian 更轻量的本地 Markdown 桌面工具。",
             Inches(1.05), Inches(6.42), Inches(9.7), Inches(0.42),
             font=SANS, size=13, color=BRAND)
    add_text(slide, "01",
             Inches(11.55), Inches(6.82), Inches(0.7), Inches(0.25),
             font=SANS, size=10, color=STONE,
             align=PP_ALIGN.RIGHT)

    prs.save('../markdown-press-feature-slide.pptx')
