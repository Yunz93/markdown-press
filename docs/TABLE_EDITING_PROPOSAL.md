...
文档状态：可评审草案  
关联：[PRD](./PRD.md) §6.2 / FR-101–106 / FR-205  
...
### Phase C — Live 单元格叠层（P2，可选）

仅当 Phase A/B 稳定且用户仍反馈「源码管道难读」时再做。

1. CodeMirror `Decoration` / `WidgetType`：将表格块替换为只读网格，点击单元格进入编辑。
2. 编辑态写回对应 cell 文本，整表 re-serialize。
3. 默认关闭或仅分屏/预览侧提供「表格焦点编辑」；源码模式保持 Phase A。
4. **不**引入完整富文本引擎。

风险：大表 widget 性能、与 Wiki/公式 inline 装饰冲突、IME 输入。需单独技术 Spike。
